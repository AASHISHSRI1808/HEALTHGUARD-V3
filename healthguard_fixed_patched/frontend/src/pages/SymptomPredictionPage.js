    ...
    <div className="page-container" style={{background:'var(--bg-base)'}}>
      ...
      <div className="content-wrapper">
        ...
        <div style={{textAlign:'center',marginBottom:'32px'}}>
          ...
          <p style={{color:'var(--text-muted)',fontSize:'14px',lineHeight:1.7,maxWidth:'560px',margin:'0 auto',fontWeight:500}}>Select symptoms → get recommended tests → upload your report → receive a structured clinical analysis.</p>
        </div>
          ...
          <div className="card animate-fadeInUp" style={{borderTop:'3px solid #84CC16'}}>
            <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'20px',flexWrap:'wrap',gap:'12px'}}>
              <div>
                ...
                <p style={{color:'var(--text-muted)',fontSize:'13px',fontWeight:500}}>Our local OCR engine will extract text and generate a structured clinical report.</p>
              </div>
            ...
            <div onClick={()=>fileRef.current.click()} style={{border:`2.5px dashed ${fileError?'#ef4444':uploadedFile?'#84CC16':'var(--border)'}`,borderRadius:'16px',padding:'38px 24px',textAlign:'center',cursor:'pointer',background:fileError?'rgba(239,68,68,0.04)':uploadedFile?'rgba(132,204,22,0.05)':'var(--bg-raised)',transition:'all 0.25s'}}
              ...
              {uploadedFile ? (
                ...
                <><Upload size={36} color="var(--text-muted)" style={{marginBottom:'12px'}}/><div style={{fontWeight:800,color:'var(--text-secondary)',fontSize:'15px',marginBottom:'6px',fontFamily:'Outfit,sans-serif'}}>Drop your medical report here</div><div style={{fontSize:'12px',color:'var(--text-muted)',fontWeight:500,lineHeight:1.6}}>Lab reports, ECG, scans · PDF, JPG, PNG · Max 10 MB</div><div style={{marginTop:'10px',fontSize:'11px',color:'var(--text-muted)',background:'var(--bg-surface)',padding:'4px 12px',borderRadius:'16px',display:'inline-block',border:'1px solid var(--border)'}}>Local OCR extracts values — works offline, no API key</div></>
              )}
            </div>
